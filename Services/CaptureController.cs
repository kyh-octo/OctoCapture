...
{
    /// <summary>캡쳐 방식. 오버레이의 모드 바에서 전환할 수 있다.</summary>
    public enum CaptureMode { Region, Window, Unit, Monitor, Full, Scroll }

    /// <summary>
    /// 모든 캡쳐 모드의 진입점. 단축키/트레이/메인 창에서 호출된다.
    /// 캡쳐 오버레이 상단에 모드 바가 표시되어 방식 전환/취소가 가능하며,
    /// 전환해도 프리즈 프레임(정지 화면)은 유지된다.
    /// </summary>
    public class CaptureController
    {
        ...
        /// </summary>
        private async Task InteractiveCaptureAsync(CaptureMode mode)
        {
            ...
            while (true)
            {
                switch (mode)
                {
                    case CaptureMode.Region:
                    {
                        ...
                        win.ShowDialog();
                        if (win.SwitchRequest is CaptureMode next) { mode = next; continue; }
