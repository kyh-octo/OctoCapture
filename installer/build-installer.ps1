...
if (-not $SkipWebsite) {
    ...
    try {
        & (Join-Path $PSScriptRoot "update-website.ps1") -AppName "OctoCapture" -Version $version -InstallerPath $setup -CoAuthor $CoAuthor
    } catch {
        Write-Host "경고: 웹사이트 갱신 실패 - $($_.Exception.Message)" -ForegroundColor Red
        Write-Host "      수동 실행: powershell -ExecutionPolicy Bypass -File installer\update-website.ps1 -AppName OctoCapture -Version $version -InstallerPath `"$setup`"" -ForegroundColor Red
    }
}
